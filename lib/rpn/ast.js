...
exports.Variable.prototype.compile = function(data) {
    return this._sn(data.originalFilename, push(["__rpn.scope.", this._name]));
}
    ...
    switch (this._operator.symbol) {
        case 'print':
            return output
                ...
                .add(["if (Math.floor(", temp, ") != ", temp, ") throw new Error('argument must be an interge');\n"])
                .add(["__rpn.", this._operator.compile(data), "(__rpn.pop(), ", temp, ");\n"]);
        case '=':
            return output
                .add(this._right.compile(data))
                .add(this._left.compileReference(data))
                .add(["__rpn.scope[__rpn.pop()] ", this._operator.compile(data), " __rpn.pop(); \n"])
        case '/':
            return output
                .add(this._left.compile(data))
                .add(this._right.compile(data))
